...
export const PackItems: React.FC<PackItemProps> = ({
  items,
  removeItem,
  updateItem,
  weightUnit,
  ...
  const onDragEndItem = (result: any) => {
    ...
    // if we dropped it in our list
    if (source.droppableId === destination.droppableId) {
      const sortedItemsInCategory = reorderItem(
        items
          .sort(sortItems)
          .filter((i) => i.categoryId.toString() === source.droppableId),
        source.index,
        destination.index
      )
      let allItems: PackItem[] = items
        .sort(sortItems)
        .filter((i) => i.categoryId.toString() !== source.droppableId) // remove those old items from the main list
  ...
  const reorderItem = (
    ...
    result.forEach((item) => {
      // then assign the sort order based on the indexes we have now
  ...
  const onDragEndCategory = (result: any) => {
    ...
    if (source.droppableId === destination.droppableId) {
      // if we dropped it in our list
      const reorderedItems = reorderCategory(
  ...
  const reorderCategory = (
    ...
    sortedGroupCategories = sortedGroupCategories.filter(
      (cat) => cat.packItems.length > 0
    )
  ...
  const sortItems = (a: PackItem, b: PackItem) => {
    return a.packItem && b.packItem
      ? a.packItem.sort_order - b.packItem.sort_order
      : 0
  }
  ...
  const sortCategories = (a: Category, b: Category) => {
    ...
    return catAItem && catBItem
      ? catAItem.packItem.sort_order - catBItem.packItem.sort_order
      : -1
  }
    ...
    categories.sort(sortCategories).map((cat, catIndex) => {
      ...
      const Header = (
        <>
          <h3>{cat.name}</h3>
          <strong>
            {catWeight!.total.label} {weightUnit}
          </strong>
        </>
      )
      return (
          ...
          {(provided) => (
            <div ref={provided.innerRef} {...provided.droppableProps}>
              {!!items.length ? (
                ...
                <MessageArea>
                  Select items from your inventory to begin
                  <br /> building your packing list.
                </MessageArea>
              )}
              {provided.placeholder}
            </div>
